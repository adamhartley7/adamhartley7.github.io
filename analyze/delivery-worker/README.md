...
The research-safe v1 and v2 top-level schemas remain unchanged. V1 accepts truthfully reconciled source contracts for Claude Code, Codex, Claude Chat, ChatGPT, Anthropic Console, Cursor, and GitHub Copilot. Cursor reports use exported token and cost columns where present. Copilot reports meter premium requests and AI credits, never tokens. V2 is the exact v1 top-level object plus `timeline`, `session_distributions` and `workflow_shape`. It accepts only the vetted v2 local collector and parser versions for Claude Code or Codex safe-usage exports. Value-model member acceptance is intentionally narrowed during the transition described below.

## Cursor and Copilot activation blockers

The Worker recognizes the exact Cursor and Copilot source, measurement, coverage, activity, cost, pricing, and questionnaire contracts. Questionnaire context remains optional. When it is present, its selected source must match the report source.

Cursor and Copilot model labels are checked against finite, source-specific, case-insensitive allowlists before any email can be prepared. One `Auto: ` wrapper is supported only for an already allowed Copilot model. Unknown or future public model names fail closed until the allowlist receives a separate review. This is intentional because the current frontend model sanitizer can preserve a private-looking label that happens to match a broad model-family pattern. Cursor estimates are recomputed from their aggregate tokens and the matching reviewed or user-edited rate. A model row that mixes recorded and estimated dollars is rejected because the current safe schema does not expose enough information to verify the split.

The Worker also rejects the current frontend's false missing-cost state. A fully missing unpriced Cursor or Copilot export can currently be serialized as `partial` with `$0` instead of `unavailable` with a null amount. The Worker will not email that claim. It also reconciles usage events, cost rows, model rows, applied-rate provenance, and supported file counters before accepting a report. Cursor and Copilot activation therefore remains blocked until the frontend preserves null missing costs, emits only reviewed public model labels, provides a verifiable state for mixed-cost model rows or keeps them local, and the combined exact reports pass this Worker suite. This change does not modify those Claude-owned analyzer parsers.
